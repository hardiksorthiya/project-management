                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div class="card-tools">
                                <a href="{{ route('projects.task.index', $project->id) }}" class="btn btn-sm btn-primary float-start">Back</a>
                            </div>
                        </div>

                        <div class="card-body">
                            <form action="{{ isset($task) ? route('task.update', $task->id) : route('projects.tasks.store', $project->id) }}" method="POST">
                                @csrf
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        @error('due_at') <div class="text-danger">{{ $message }}</div> @enderror
                                    </div>
                                </div>
                                    ...
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <select name="priority" class="form-select">
                                            @foreach(['low', 'medium', 'high'] as $level)
                                                <option value="{{ $level }}" {{ old('priority', $task->priority ?? '') === $level ? 'selected' : '' }}>{{ ucfirst($level) }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-4 mb-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="parent_id" class="form-select">
                                        ...
                                        @foreach($tasks as $parent)
                                            <option value="{{ $parent->id }}" {{ old('parent_id', $task->parent_id ?? '') == $parent->id ? 'selected' : '' }}>{{ $parent->title }}</option>
                                        @endforeach
                                    </select>
                                </div>
